...
const profileUrl=
`https://srm.nhso.go.th/api/right-search/personal-id?personalId=${pid}`;

const historyUrl=
`https://srm.nhso.go.th/api/right-search/history?personalId=${pid}`;
...
type:'text',
text:'❌ กรุณากรอกเลข 13 หลัก\nตัวอย่าง: h%1342200084655'
});
}

try{

const data=await fetchNHSOFull(pid);

const p=data.profile?.personalData;

const currentFund=
data.profile?.personalFundList?.[0];
...
const historyText=historyList.length

? historyList.map((item,index)=>{

const mainHosp=
item.hospMain?.displayName ||
item.hospSss?.displayName ||
"-";

const subHosp=
item.hospSub?.displayName ||
"-";

return `📌 รายการที่ ${index+1}
├● วันที่เริ่มสิทธิ์: ${formatThaiDate(item.startDateTime)}
├● วันที่สิ้นสุด: ${formatThaiDate(item.expireDateTime)}
├● สถานะ: ${item.status?.shortDesc||"-"}
├● สิทธิ์หลัก: ${item.mainInscl?.rightName||"-"}
├● สิทธิ์ย่อย: ${item.subInscl?.insclName||"-"}
├● จังหวัด: ${item.province?.name||"-"}
├● หน่วยบริการหลัก: ${mainHosp}
├● หน่วยบริการรอง: ${subHosp}
└● วันที่ปรับปรุง: ${formatThaiDate(item.transDate)}`;
...
👤ข้อมูลบุคคล
├● ชื่อ: ${p.fullName||"-"}
├● เลขบัตร: ${p.personalId||pid}
├● เพศ: ${p.sexDesc||"-"}
├● อายุ: ${p.age?.years||"-"} ปี
└● วันเกิด: ${p.displayBirthDate||"-"}

📑สิทธิ์ปัจจุบัน
├● สถานะ: ${currentFund?.status?.shortDesc||"-"}
├● สิทธิ์หลัก: ${currentFund?.mainInscl?.rightName||"-"}
├● หน่วยบริการหลัก: ${currentFund?.hospMain?.displayName||"-"}
└● หน่วยบริการรอง: ${currentFund?.hospSub?.displayName||"-"}

━━━━━━━━━━━━━━
📚 ประวัติสิทธิ์ย้อนหลัง
━━━━━━━━━━━━━━
...
🧑‍🦲 เปรียบเทียบใบหน้า
└ ff% และ รอระบบแจ้งให้ส่งภาพใบหน้า
🚗 อ่านป้ายทะเบียน และ วิเคราะห์รถ
└ pt% และ รอระบบแจ้งให้ส่งภาพ

⚡ ไฟฟ้า / ข้อมูลอื่นๆ
├ ศูนย์บริการรถ bq%ชื่อ / เบอร์โทร / เลขบัตร
┣ ไฟนครหลวง mea%ชื่อสกุล
┣ ไฟนครหลวง kru%เลขมิเตอร์
┣ ไฟภูมิภาค peab%เลข CA เลขมิเตอร์
┣ ไฟภูมิภาค peac%เลข CA
